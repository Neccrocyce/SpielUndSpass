...
public class MainFrame extends Application {
	...
	private static double xRes, yRes;
	...
	private Text[][][] teamPlayerNameText;		//playerName and -score
	...
	private Font createFontPaperHeading () {
		return createFontPaper(gameText[0].getText(), gameText[0].getPrefWidth(), height, teamScoreText[0][0].getFont().getSize());
	}
	...
	private Font createFontPaperBottom () {
		return Font.font("Old English Text MT", FontWeight.NORMAL, (2.0/3.0) * gameText[0].getFont().getSize());
	}
	...
	public void resizeText() {		
		...
		gameTimerText[0].setFont(createFontPaperBottom());
		gameTimerText[1].setFont(createFontPaperBottom());
	...
	/**
	 * 
	 * @param args 0: numOfPlayers : Integer, 1: resolutionX: double, resolutionY: double
	 * @throws IllegalArgumentException
	 */
	public static void initialize (String[] args) throws IllegalArgumentException {
		try {
			numOfPlayers = Integer.parseInt(args[0]);
			numOfPlayers = (numOfPlayers % 2) == 0 ? numOfPlayers / 2 : numOfPlayers / 2 + 1;
			xRes = Double.parseDouble(args[1]);
			yRes = Double.parseDouble(args[2]);
			
			if (numOfPlayers <= 0 || xRes <= 0 | yRes <= 0) {
	 ...
	 * player (int): 0: Player1; ...
	 */
	
	public void setTeamName (int team, String name) {
		teamNameText[team].setText(name);
		resizeText();
	}
	
	public void setVicPoints (int team, int score) {
		int ones = score % 10;
		int tenths = (score / 10) % 10;
		teamScoreText[team][0].setText("" + tenths);
		teamScoreText[team][1].setText("" + ones);
		resizeText();
	}
	
	/**
	 * sets score of team of current game
	 * @param team
	 * @param score
	 */
	...
	public void setPlayerName (int team, int player, String name) {
		teamPlayerNameText[team][player][0].setText(name.replaceAll("", "\n").trim());
		resizeText();
	}
	
	public void setPlayerScore (int team, int player, int score) {
		teamPlayerNameText[team][player][1].setText("" + score);
		resizeText();
	}
	
	public void setPlayerText (int team, int player, String text) {
		Platform.runLater(() -> {teamPlayerTxtText[team][player].setText(text);});
		resizeText();
	}
	
	public void setGameTitel (String text) {
		Platform.runLater(() -> {gameText[0].setText(text);});
		resizeText();
	}
	
	public void setGameText (String text) {
		Platform.runLater(() -> {gameText[1].setText(text);});;
		resizeText();
	}
	
	public void setTimer(int timer, int h, int m, int s) {
		String time = (h > 9 ? "" : "0") + h;
		time += (m > 9 ? ":" : ":0") + m;
		time += (s > 9 ? ":" : ":0") + s;
		final String time2 = time;
		Platform.runLater(() -> {gameTimerText[timer].setText(time2);});		
		resizeText();
	}
	
	public void setVisibleTimer (int timer) {
		gameTimerText[timer].setOpacity(1.0);
	}
	
	public void setInvisibleTimer (int timer) {
		gameTimerText[timer].setOpacity(0.0);
	}	
=======
import javafx.beans.value.ChangeListener;
import javafx.beans.value.ObservableValue;
